...
module sensor_scheduler_chain_tb;
    ...
    // monitor sens_id stability during one transaction
    logic trans_active;
    ...
    always @(posedge clk) begin
        if (reset) begin
            prev_sched_state <= u_sched.state;
            ...
            if (u_sched.state != prev_sched_state ||
    ...
    // =========================================================
    // slave sequences for each sensor
    // =========================================================
    task automatic run_ads_once();
        ...
        begin
            ...
            // write config: ADDR_W, REG_CONFIG, CFG_HI, CFG_LO
            ...
            // read conv: ADDR_W, REG_CONV, ADDR_R
            wait_ack_phase_and_drive(1'b0);
            wait_ack_phase_and_drive(1'b0);
            wait_ack_phase_and_drive(1'b0);
    ...
    // =========================================================
    // TEST 1: automatic round robin
    // =========================================================
    task automatic test_auto_round_robin();
        ...
        begin
                ...
                begin : NORMAL_PATH
                    fork
                        ...
                        begin : CHECK_THREAD
                            ...
                            check(result_error == 1'b0, "ADS #1 result_error remains 0");
                            check(result_data == exp_ads, "ADS #1 result_data correct");
                            @(posedge clk);
                            ...
                            // ADS #2 wrap
                            wait(result_valid == 1'b1 && result_sensor_id == S_ADS1115);
                            check(result_error == 1'b0, "ADS #2 result_error remains 0");
                            check(result_data == exp_ads, "ADS #2 result_data correct");
                        end
                    join
                end

                begin : TIMEOUT_THREAD
                    repeat (2300000) @(posedge clk);
            ...
            @(posedge clk);
            #1;
            check(complete_count >= 4, "at least 4 automatic completions observed");
        end
    endtask
        ...
        begin
            ...
            fork
                ...
                begin : CHECK_THREAD
                    ...
                    // should still complete current sensor
                    wait(result_valid == 1'b1 && result_sensor_id == S_ADS1115);
                    check(result_error == 1'b0, "current ADS sample still completes after stop");

                    // after current sample completes, scheduler should go idle and no more requests
                    repeat (20) @(posedge clk);
                    check(u_sched.state == u_sched.S_IDLE, "scheduler returns to IDLE after safe stop");
