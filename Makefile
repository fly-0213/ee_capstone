...
# the CSEE 4340 synthesis script
TCL_SCRIPT = csee4340_synth.tcl
...
TESTBENCH = test/sensor_scheduler_packetizer_fifo_serializer_tb.sv
SOURCES = verilog/pkt_serializer.sv\
		  verilog/fifo.sv \
		  verilog/scheduler_os.sv \
		  verilog/ads1115_ctrl.sv \
		  verilog/sht30_ctrl.sv \
		  verilog/mpl3115_ctrl.sv \
		  verilog/sensor_i2c_mux.sv \
		  verilog/sensor_result_mux.sv \
		  verilog/packetizer.sv \
		  verilog/i2c_master.sv \
		  verilog/time.sv
//TESTBENCH   = test/pkt_serializer_tb.sv
//SOURCES     = verilog/pkt_serializer.sv verilog/time.sv
MODULE_TOP  = 
...
$(MODULE_TOP).vg: $(SOURCES) | $(TCL_SCRIPT)
	@$(call PRINT_COLOR, 5, synthesizing the $* module)
	@$(call PRINT_COLOR, 3, this might take a while...)
	@$(call PRINT_COLOR, 3, NOTE: if this is slow to startup: run '"module load vcs verdi synopsys-synth"')
	# pipefail causes the command to exit on failure even though it's piping to tee
...
verdi_dir:
	mkdir -p /workdir/$${USER}_csee_4340
.PHONY: verdi_dir
